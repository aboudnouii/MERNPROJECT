...
// GET /api/repairs/track/:trackingCode
router.get("/track/:trackingCode", async (req, res) => {
  try {
    const trackingCode = String(req.params.trackingCode || "").trim().toUpperCase();

    const repairJob = await RepairJob.findOne({ trackingCode }).select(
      "trackingCode customerName carModel licensePlate issueDescription status estimatedCost actualCost createdAt completedAt"
    );

    if (!repairJob) {
      return res.status(404).json({ message: "No repair job found for this tracking code" });
    }

    res.json(repairJob);
  } catch (error) {
    res.status(500).json({ message: "Failed to track repair job", error: error.message });
  }
});

// All routes below are private garage staff routes.
router.use(protect);
...
router.put("/:id", async (req, res) => {
  try {
    ...
    delete updateData.trackingCode; // tracking code must stay stable for customer privacy/linking
