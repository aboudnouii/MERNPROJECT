...
function CustomerTrack({ onBackToLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Unable to find this repair code. Please check the code and try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main className="customer-track-page">
      <section className="track-hero">
        ...
        <h1>Track Your Car Repair</h1>
        <p>Enter the tracking code given by the garage to see your vehicle repair status without creating an account.</p>
      </section>

      <section className="track-card">
        <button className="link-btn" onClick={onBackToLogin}>← Staff login</button>
        <h2>Repair Status Lookup</h2>
        <p className="muted">Example code format: RM-260424-A1B2C3</p>
          ...
          <article className="customer-result">
            <div className="repair-main">
              <div>
                ...
                <p className="muted">Tracking Code: <strong>{repair.trackingCode}</strong></p>
              </div>
            ...
            <div className="meta-grid">
              ...
              {repair.completedAt && <span><strong>Completed:</strong> {new Date(repair.completedAt).toLocaleDateString()}</span>}
            </div>
          </article>
        )}
      </section>
